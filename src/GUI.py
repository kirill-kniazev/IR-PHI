...
# Import hardware control classes
from madpiezo import Madpiezo           # Piezo manipulator controll class
...
import zhinst.ziPython, zhinst.utils    # Zurich Instruments lock-in controll class
...
class PiezoManipulation(tk.Frame):
    def __init__(self, master):
        ...
        self.WAVENUM_RIGHT_BORDER = 1840 # wavenumber, cm⁻¹

        self.MIRROR_LEFT_BORDER = 0 #lingth of the rode, mm
        self.MIRROR_RIGHT_BORDER = 3 #lingth of the rode, mm; REAL BORDER IS 6MM. 3mm limited digitally due to the mount restrictions
        
        #* FORM VARIABLES (VALUES BY DEFAULT)
        self.filter_slope_var = tk.IntVar(value=8) 
        self.time_constant_var = tk.DoubleVar(value=0.1)
        ...
        self.mirror_position = tk.DoubleVar(value=1.582) #for initial mirror position

        # for mirror corection for spectra
        self.mirror_correction_var = tk.IntVar(value=1) 

        #* MIRROR CORRECTION MAP FOR SPECTRA; SORT IS NOT NECESSARY
        #* MOTOR STEPS REPRESENTS DISTANCE FROM ZERO (INTIAL) POSITION
        self.mirror_correction_map = [
            ...
            {"wavenum" : 1842, "motor_step" : 1.545, "sleep_time" : 1}
        ]        
        ...
        ## LABEL MIRROR
        self.mirror_lab_lf1 = ttk.Label(self.lf1,
                                    text="d, mm (" +
                                    str(self.MIRROR_LEFT_BORDER) +
                                    "-" +
                                    str(self.MIRROR_RIGHT_BORDER) +
                                    "): ")
        self.mirror_lab_lf1.grid(column=0, 
                            row=3,
                            padx=5,
                            pady=5,
                            sticky="E")
        ...
        ## SPINBOX MIRROR
        self.mirror_spin_lf1 = ttk.Spinbox(self.lf1, 
                                      from_=self.MIRROR_LEFT_BORDER, 
                                      to=self.MIRROR_RIGHT_BORDER, 
                                      width=10)
        self.mirror_spin_lf1.grid(column=1, 
                             row=3,
                             padx=5,
                             pady=5,
                             sticky="E")
        ...
        self.mirror_lab_lf2 = tk.Label(self.lf2, text="d, mm: ", bg=bg_color)
        self.mirror_lab_lf2.grid(column=0,
                            row=4,
                            padx=5,
                            sticky="E")
        ...
        ## LABEL MIRROR COORD
        self.mirror_coord_lab_lf2 = tk.Label(self.lf2, text="---", bg=bg_color)
        self.mirror_coord_lab_lf2.grid(column=1, 
                                  row=4,
                                  padx=5,
                                  sticky="E")
        ...
        ## LABEL ΔX
        self.delta_x_lab_fr_im = ttk.Label(self.imaging_frame, text="ΔX, μm (min 0.025): ")
        ...
        ## LABEL ΔY
        self.delta_y_lab_fr_im = ttk.Label(self.imaging_frame, text="ΔY, μm (min 0.025): ")
...
hardware and software")
    ...
    def read_position_mirror (self):
        """
        Read and write position of the mirror

        """
        self.mirror_coord_lab_lf2["text"] = round(self.mirror.read_cur_pos(),3)
    ...
    def d_is_in_proper_range(self, d):
        """
        Checks that mirror coords are in proper range.

        """
        flag = ((self.MIRROR_LEFT_BORDER <= d <= self.MIRROR_RIGHT_BORDER))
        return flag
    ...
    def mirror_go_to_position(self, d):
        """
        1. Set absolute position for mirror
        2. Then some "sleep" to reach the position.
        3. Read and write current position.
        """
        self.mirror.move_absolute(d)
        sleep(self.SLEEP_TIME)
        self.read_position_mirror() #read and write current position
    ...
    def on_initialize(self):
        ...
        if text == "Initialize":
            try:
                ...
                #initialise mirror motor
                self.mirror = ConexCC(com_port='COM3', velocity=0.4)
                ...
                if self.piezo.handler != 0: # is good
                    ...
                    self.mirror.wait_for_ready(timeout=60)
                    self.mirror_go_to_position(1.582) #this spin box is filled up initially
    ...
    def on_go(self):
        ...
        try:    
            ...
            d = round(float(self.mirror_spin_lf1.get()), 3)
        except Exception:
            showwarning(message="Bad values! Enter numbers.")
            return -1
        
        # check if values for stage are in proper ranges  
        if self.xyz_is_in_proper_range(x, y, z):
            self.piezo_go_to_position(x, y, z)
        else:
            showwarning(message="Values for stage must be in proper ranges! Try again.")

        # check if values for mirror are in proper ranges  
        if self.d_is_in_proper_range(d):
            self.mirror_go_to_position(d)
        else:
            showwarning(message="Values for mirror must be in proper ranges! Try again.")
    ...
    def on_save_file(self):
        ...
        if filename:
            try:
                ...
                ext = ".csv"
                delta_x = str( round( float(self.delta_x_spin_fr_im.get()), 2) )
                delta_y = str( round( float(self.delta_y_spin_fr_im.get()), 2) )
                area_step = f"[{str(self.x2 - self.x1)}x{str(self.y2 - self.y1)}um-step{delta_x}x{delta_y}um]"

                if self.experiment_type == "Imaging":
                    dir_name = "C:\\Users\\kuno\\OneDrive - nd.edu\\\
Documents\\Measurements\\Maps\\"
                    ...
                    if need_save_r and need_save_theta:
                        ...
                        full_name_r = (dir_name + date + area_step + filename + 
                                       postfix_r + ext)
                        np.savetxt(full_name_r, self.r_data, delimiter=",")
                        
                        
                        full_name_theta = (dir_name + date + area_step + filename +
                                           postfix_theta + ext)
...
Documents\\Measurements\\Spectra\\"
    ...
    def on_imag_apply_parameters(self):
        ...
        if self.initialized: # if piezo initialized
            ...
            # go to wavelength corresponding mirror position
            self.mirror_go_to_position_wevelength_corection(wavenum)
...
On average {round(total_time_min * 1000 * 60 / length, 2)} ms per step.")
    ...
    def on_spec_apply_parameters(self):
        ...
        if self.initialized:
            ...
            # go to wavelength corresponding mirror position
            self.mirror_go_to_position_wevelength_corection(wavenum1)
    ...
    def on_spec_start(self):
        ...
        """
        print("x_coord value:", self.x_coord_lab_lf2.cget("text"))
        print("y_coord value:", self.y_coord_lab_lf2.cget("text"))
        print("mirror_spin value:", self.mirror_spin_lf1.get())
        ...
        if need_mirror_correction:
            try: 
                self.mirror_go_to_position(float( self.mirror_spin_lf1.get() ))
        ...
        # move the wavenumber over the desired range
        for step, index in enumerate(np.ndindex(scan_shape)):
            try:
                t1 = time() # start time
...
On average {round(total_time_min * 1000 * 60 / scan_shape, 2)} ms per step.")
        ...
        if need_mirror_correction:
            ...
            top.title("")
            lab = tk.Label(top, text = "Mirror is going back...")
    ...
    def mirror_correction(self, wavenum_val):
        ...
        if wavenum_val >= wavenum_border:
            ...
            self.mirror_go_to_position(float(dict_val["motor_step"])) #this spin box is filled up initially
